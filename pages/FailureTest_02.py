...
#     page = st.radio("选择页面", ["数据加载", "故障检测", "知识图谱", "诊断记录"])



try:
    df = pd.read_csv(DATA_PATH)
    signal_col = st.selectbox("选择信号列", df.columns)

    if st.button("开始检测"):
        with st.spinner("处理中..."):
            signal = df[signal_col].dropna().values[:1024]
            features = extract_features(signal)
            kg = load_kg()
            fault, reason, solution = kg_reasoning(features, kg)
            llm_res = llm_diagnose(features, fault)
