...
if __name__ == '__main__':
    ...
    _, btn_gen_1, btn_sub_1 = st.columns([10, 1, 1])  # 这里的比例 [10, 1, 1] 可以根据按钮宽度微调，确保按钮不被挤压
    with btn_gen_1:
    ...
    # st.markdown("""
    #     **本体定义示例：**
    #     - 类（Classes）：Person, Animal, Vehicle
    #     - 属性（Properties）：name, age, color
    #     - 关系（Relationships）：hasParent, hasChild, owns
    # """)

    ##########################################
    st.subheader("4. 实体抽取")
    default_template = """轴承峭度很大，判定为内圈故障，原因是润滑不足，建议更换轴承。"""
    ...
    _, btn_gen_2 = st.columns([10, 1])  # 这里的比例 [10, 1] 可以根据按钮宽度微调，确保按钮不被挤压
    with btn_gen_2:
    ...
    if gen_2:
        with st.spinner('正在分析实体间的关系...'):
            ...
            仅输出 JSON 格式
            {
            {"头实体": "x", "头类型": "x", "关系": "x", "尾实体": "x", "尾类型": "x", "置信度": "x"},
            {...}
            }"""
            ...
            for item in response:
                new_relations.append({"source": item['头实体'], "target": item['尾实体'], "relation": item['关系']})
    ...
    if in_database:
        with st.spinner('正在入库...'):
            try:
                ...
                with open('./data/kg.json', 'r', encoding='utf-8') as file:
                    data = json.load(file)
